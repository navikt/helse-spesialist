...
class SaksbehandlerMediatorTest : AbstractDatabaseTest() {
    ...
    fun `ny overstyring uten eksisterende totrinnsvurdering lager totrinnsvurdering`() {
        ...
        checkNotNull(totrinnsvurdering)
        assertTrue(
            totrinnsvurdering.overstyringer
                .single()
                .opprettet
    ...
    fun `håndterer overstyring av tidslinje`() {
        ...
        assertEquals(overstyringId.toString(), hendelse["@id"].asString())
        assertEquals(person.fødselsnummer, hendelse["fødselsnummer"].asString())
        assertEquals(person { 2.ag }, hendelse["organisasjonsnummer"].asString())

        val overstyrtDag = hendelse["dager"].toList().single()
        assertEquals(10.januar, overstyrtDag["dato"].asLocalDate())
        assertEquals("Sykedag", overstyrtDag["type"].asString())
        assertEquals("Arbeidsdag", overstyrtDag["fraType"].asString())
    ...
    fun `håndterer overstyring av arbeidsforhold`() {
        ...
        assertNotNull(hendelse["@id"].asString())
        assertEquals(FØDSELSNUMMER, hendelse["fødselsnummer"].asString())
        assertEquals(AKTØR_ID, hendelse["aktørId"].asString())
        assertEquals(SAKSBEHANDLER_OID, hendelse["saksbehandlerOid"].asString().let { UUID.fromString(it) })
        assertEquals(SAKSBEHANDLER_NAVN, hendelse["saksbehandlerNavn"].asString())
        assertEquals(SAKSBEHANDLER_IDENT.value, hendelse["saksbehandlerIdent"].asString())
        assertEquals(SAKSBEHANDLER_EPOST, hendelse["saksbehandlerEpost"].asString())
        ...
        assertEquals("en begrunnelse", overstyrtArbeidsforhold["begrunnelse"].asString())
        assertEquals("en forklaring", overstyrtArbeidsforhold["forklaring"].asString())
        assertEquals(ORGANISASJONSNUMMER_GHOST, overstyrtArbeidsforhold["orgnummer"].asString())
    ...
    fun `håndterer overstyring av inntekt og refusjon`() {
        ...
        hendelse["arbeidsgivere"].first().let {
            assertEquals(ORGANISASJONSNUMMER, it["organisasjonsnummer"].asString())
            assertEquals("En begrunnelse", it["begrunnelse"].asString())
            assertEquals("En forklaring", it["forklaring"].asString())
            assertEquals(25000.0, it["månedligInntekt"].asDouble())
            assertEquals(2, it["refusjonsopplysninger"].size())
            assertEquals("2018-01-01", it["refusjonsopplysninger"].first()["fom"].asString())
            assertEquals("2018-01-31", it["refusjonsopplysninger"].first()["tom"].asString())
        ...
        hendelse["arbeidsgivere"].last().let {
            assertEquals(ORGANISASJONSNUMMER_GHOST, it["organisasjonsnummer"].asString())
            assertEquals("En begrunnelse 2", it["begrunnelse"].asString())
            assertEquals("En forklaring 2", it["forklaring"].asString())
