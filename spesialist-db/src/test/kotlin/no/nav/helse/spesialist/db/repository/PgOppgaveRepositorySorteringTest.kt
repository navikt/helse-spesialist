...
class PgOppgaveRepositorySorteringTest : AbstractDBIntegrationTest() {
    ...
    private fun hentOppgaver(
        ...
        side: Int = 1,
        antallRader: Int = 10,
    ): OppgaveRepository.Side<OppgaveRepository.OppgaveProjeksjon> =
    ...
    fun finnTildeltTilNavn(oppgaveIds: List<Long>) =
        dbQuery.list(
            """
            ...
            """.trimIndent(),
            "ider" to oppgaveIds.somDbArray(),
        ) {
            it.string("navn")
        }

    fun finnPåVentId(oppgaveIds: List<Long>) =
        oppgaveIds.map { oppgaveId ->
            dbQuery.single(
                """
