...
internal class PgAbonnementApiDaoTest : AbstractDBIntegrationTest() {
    ...
    fun `får nytt sekvensnummer selvom det allerede fins et`() {
        settSekvensnummer(saksbehandler.id.value, 42)
        ...
        assertEquals(962, finnSekvensnummer(saksbehandler.id.value))
    }
    ...
    fun `får satt nyeste globale sekvensnummer hvis verken person eller saksbehandler har noen`() {
        ...
        assertEquals(endaEtSekvensnummerForEnUrelatertPerson, finnSekvensnummer(saksbehandler.id.value))
    }
    ...
    fun `alle tidligere abonnement slettes når et nytt opprettes`() {
        ...
        abonnementDao.opprettAbonnement(saksbehandler.id.value, person1.aktørId)
        assertEquals(listOf(person1.aktørId), finnPersonerSaksbehandlerAbonnererPå(saksbehandler.id.value))
        abonnementDao.opprettAbonnement(saksbehandler.id.value, person2.aktørId)
        assertEquals(listOf(person2.aktørId), finnPersonerSaksbehandlerAbonnererPå(saksbehandler.id.value))
    }
    ...
    fun `fungerer også for personer med flere fødselsnumre`() {
        ...
        assertDoesNotThrow {
            abonnementDao.opprettAbonnement(saksbehandler.id.value, aktørId1)
        }
    }
