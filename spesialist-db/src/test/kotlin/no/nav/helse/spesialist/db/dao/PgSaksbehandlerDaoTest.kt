...
class PgSaksbehandlerDaoTest : AbstractDBIntegrationTest() {
    ...
    fun `henter saksbehandler for ident`() {
        ...
        val resultat = saksbehandlerDao.hent(saksbehandler.ident.value)
        assertNotNull(resultat)
        assertEquals(saksbehandler.ident, resultat?.ident)
    }

    @Test
    fun `ukjent saksbehandler-ident retunerer null`() {
        val resultat = saksbehandlerDao.hent(lagSaksbehandler().ident.value)
        assertNull(resultat)
    }
