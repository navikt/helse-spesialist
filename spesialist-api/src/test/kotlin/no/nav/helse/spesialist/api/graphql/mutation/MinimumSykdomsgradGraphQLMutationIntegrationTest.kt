...
import no.nav.helse.spesialist.api.testfixtures.mutation.asGQL
...
class MinimumSykdomsgradGraphQLMutationIntegrationTest {
    ...
    fun `Bad request dersom request ikke inneholder noen vurderte perioder`() {
                        ...
                        arbeidsgivere: [
                            {
                                ...
                                berortVedtaksperiodeId: "$arbeidsgiver1BerortVedtaksperiodeId"
                            },
                            {
                                organisasjonsnummer: "$arbeidsgiver2Organisasjonsnummer",
                                berortVedtaksperiodeId: "$arbeidsgiver2BerortVedtaksperiodeId"
                            }
                        ],
        ...
        // Sjekk respons
        assertEquals(false, responseJson.get("data")?.get("minimumSykdomsgrad")?.asBoolean())
        assertEquals("Mangler vurderte perioder", responseJson.get("errors")?.get(0)?.get("message")?.asText())
        assertEquals(400, responseJson.get("errors")?.get(0)?.get("extensions")?.get("code")?.asInt())
    ...
    fun `Happy case`() {
        ...
        // Sjekk respons
        assertEquals(true, responseJson.get("data")?.get("minimumSykdomsgrad")?.asBoolean())
