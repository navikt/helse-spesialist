...
internal class VedtaksperiodeMediator(
    ...
    fun byggSpeilSnapshotForFnr(fnr: String) =
        measureAsHistogram("byggSpeilSnapshotForFnr") { session.findVedtakByFnr(fnr) }?.let(::byggSpeilSnapshot)

    fun byggSpeilSnapshotForAktørId(aktørId: String) =
        measureAsHistogram("byggSpeilSnapshotForAktørId") { session.findVedtakByAktørId(aktørId) }?.let(::byggSpeilSnapshot)

    fun byggSpeilSnapshotForVedtaksperiodeId(vedtaksperiodeId: UUID) =
        measureAsHistogram("byggSpeilSnapshotForVedtaksperiodeId") {
            session.findVedtakByVedtaksperiodeId(vedtaksperiodeId)
