...
class ApiTestUtil {
        ...
        companion object {
            private fun createEmbeddedServer(build: Route.() -> Unit, httpPort: Int) =
                embeddedServer(Netty, port = httpPort) {
                    ...
                    basicAuthentication(adminSecret)
                    routing {
    ...
    class TestMockServer(
        ...
        fun start(): TestMockServerRuntime {
            return TestMockServerRuntime(λ, httpPort)
        }
    ...
    class TestMockServerRuntime(
        build: Route.() -> Unit,
        private val httpPort: Int
    ) : AutoCloseable {
        private val server = createEmbeddedServer(build)

        companion object {

            private fun createEmbeddedServer(build: Route.() -> Unit): TestApplicationEngine {
                ...
                val jwkProvider = jwtStub.getJwkProviderMock()
                server.application.azureAdAppAuthentication(oidcDiscovery, azureConfig, jwkProvider)
                server.application.basicAuthentication(adminSecret)
        ...
        fun restClient(): HttpClient {
            return HttpClient(server.engine) {
                expectSuccess = false
                install(JsonFeature) {
                    serializer = JacksonSerializer(jackson = objectMapper)
                }
