...
internal class OppgaveDaoTest : AbstractEndToEndTest() {
    ...
    fun `lagre oppgave`() {
        ...
        oppgave().first().assertEquals(HENDELSE_ID, LocalDate.now(), OPPGAVETYPE, OPPGAVESTATUS, FERDIGSTIL_AV, FERDIGSTILT_AV_OID, null)
    }
    ...
    fun `oppdatere oppgave`() {
        ...
        assertEquals(1, oppgave().size)
        oppgave().first().assertEquals(HENDELSE_ID, LocalDate.now(), OPPGAVETYPE, nyStatus, null, null, null)
    }
    ...
    fun `oppdatere riktig oppgave`() {
        ...
        oppgave().first().assertEquals(HENDELSE_ID, LocalDate.now(), "EN_ANNEN_TYPE", OPPGAVESTATUS, null, null, null)
        oppgave().last().assertEquals(HENDELSE_ID, LocalDate.now(), "EN_TYPE", Oppgavestatus.Ferdigstilt, FERDIGSTIL_AV, FERDIGSTILT_AV_OID, null)
    }

    private fun oppgave() =
        using(sessionOf(dataSource)) {
            it.run(queryOf("SELECT event_id, oppdatert, type, status, ferdigstilt_av, ferdigstilt_av_oid, vedtak_ref FROM oppgave ORDER BY id DESC").map {
                Oppgave(
    ...
    private class Oppgave(
        ...
        fun assertEquals(forventetHendelseId: UUID, forventetOppdatert: LocalDate, forventetType: String, forventetStatus: Oppgavestatus, forventetFerdigstilAv: String?, forventetFerdigstilAvOid: UUID?, forventetVedtakRef: Long?) {
