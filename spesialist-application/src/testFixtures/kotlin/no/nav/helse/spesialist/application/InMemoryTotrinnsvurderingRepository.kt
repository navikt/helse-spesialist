...
class InMemoryTotrinnsvurderingRepository :
    ...
    private fun genererTotrinnsvurderingId(): TotrinnsvurderingId = TotrinnsvurderingId((alle().maxOfOrNull { it.id().value } ?: 0) + 1)

    private fun genererOverstyringId(): OverstyringId = OverstyringId((alle().flatMap { it.overstyringer }.maxOfOrNull { it.id().value } ?: 0) + 1)

    override fun deepCopy(original: Totrinnsvurdering): Totrinnsvurdering =
        Totrinnsvurdering.fraLagring(
            ...
            overstyringer =
                original.overstyringer.map { originalOverstyring ->
                    when (originalOverstyring) {
                        is MinimumSykdomsgrad -> {
                        ...
                        is SkjønnsfastsattSykepengegrunnlag -> {
                            SkjønnsfastsattSykepengegrunnlag.fraLagring(
                                ...
                                skjæringstidspunkt = originalOverstyring.skjæringstidspunkt,
                                arbeidsgivere = originalOverstyring.arbeidsgivere.toList(),
                                årsak = originalOverstyring.årsak,
                                type = originalOverstyring.type,
                                begrunnelseMal = originalOverstyring.begrunnelseMal,
                                begrunnelseFritekst = originalOverstyring.begrunnelseFritekst,
                                begrunnelseKonklusjon = originalOverstyring.begrunnelseKonklusjon,
                                lovhjemmel = originalOverstyring.lovhjemmel,
                            )
                        }
                    }
                },
