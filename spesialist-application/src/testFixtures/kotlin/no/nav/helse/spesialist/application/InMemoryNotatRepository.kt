...
class InMemoryNotatRepository :
    AbstractLateIdInMemoryRepository<NotatId, Notat>(),
    NotatRepository {
    override fun finnAlleForVedtaksperiode(vedtaksperiodeId: UUID): List<Notat> = alle().filter { it.vedtaksperiodeId == vedtaksperiodeId }

    override fun finnAlleForVedtaksperioder(vedtaksperiodeIds: Set<VedtaksperiodeId>): List<Notat> = alle().filter { VedtaksperiodeId(it.vedtaksperiodeId) in vedtaksperiodeIds }
    ...
    override fun deepCopy(original: Notat): Notat =
