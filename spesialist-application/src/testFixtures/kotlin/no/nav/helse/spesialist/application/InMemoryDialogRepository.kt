...
class InMemoryDialogRepository :
    AbstractLateIdInMemoryRepository<DialogId, Dialog>(),
    DialogRepository {
    ...
    override fun deepCopy(original: Dialog): Dialog =
        Dialog.Factory.fraLagring(
            id = original.id(),
            opprettetTidspunkt = original.opprettetTidspunkt,
            kommentarer =
                original.kommentarer.map { originalKommentar ->
