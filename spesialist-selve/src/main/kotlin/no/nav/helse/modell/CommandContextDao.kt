...
internal class CommandContextDao(private val dataSource: DataSource) {
    ...
    internal fun suspendert(hendelseId: UUID, contextId: UUID, hash: UUID, sti: List<Int>) {
    ...
    fun finnSuspendert(id: UUID) = finnSiste(id)?.takeIf { it.first == SUSPENDERT }?.let { (_, dto, hash) ->
        ...
        sessionOf(dataSource).use  { session ->
            ...
            session.run(
                queryOf(
                    ...
                    Triple(enumValueOf<CommandContextTilstand>(it.string("tilstand")),
                        mapper.readValue<CommandContextDto>(it.string("data")), it.stringOrNull("hash"))
                }.asSingle
            )
        }
