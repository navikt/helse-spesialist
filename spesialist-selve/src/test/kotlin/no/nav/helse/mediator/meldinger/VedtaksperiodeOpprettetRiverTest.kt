...
internal class VedtaksperiodeOpprettetRiverTest {
    ...
    fun `ignorerer organisasjonsnummer ARBEIDSLEDIG uten å tryne`() {
        rapid.sendTestMessage(message("ARBEIDSLEDIG"))
        verify { mediator wasNot called }
    }
    @Test
    fun `ignorerer organisasjonsnummer FRILANS uten å tryne`() {
        rapid.sendTestMessage(message("FRILANS"))
        verify { mediator wasNot called }
    }
    @Test
    fun `ignorerer organisasjonsnummer SELVSTENDIG uten å tryne`() {
        rapid.sendTestMessage(message("SELVSTENDIG"))
        verify { mediator wasNot called }
    }

    @Language("JSON")
    private fun message(organisasjonsnummer: String) =
        """
            {
