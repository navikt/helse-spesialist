...
internal class UtbetalingsfilterTest {
    ...
    @Test
    fun `endring fra full refusjon til delvis, kan utbetales`() {
    ...
    @Test
    fun `revurdering kan alltid utbetales`() {
        fun utbetalingsfilter() = Utbetalingsfilter(
            fødselsnummer = "21111111111",
            delvisRefusjon = true,
            erUtbetaltFør = true,
            ...
            periodetype = FORLENGELSE,
            inntektskilde = FLERE_ARBEIDSGIVERE,
            warnings = listOf(Warning("warning", WarningKilde.Spleis, LocalDateTime.now())),
            utbetalingtype = REVURDERING,
        )

        assertKanUtbetales(utbetalingsfilter(), false)
    }
