...
internal class OppdaterPersoninfoCommandTest {
    ...
    fun `oppdatere personinfo`() {
        ...
        verify(exactly = 1) { dao.updateOrInsertPersoninfo(FNR, FORNAVN, MELLOMNAVN, ETTERNAVN, FØDSELSDATO, KJØNN, ADRESSEBESKYTTELSE) }
    ...
    fun `oppdaterer personinfo dersom force er satt til true`() {
        ...
        assertTrue(command.execute(context))
        verify(exactly = 1) { løsning.oppdater(dao, FNR) }
        verify(exactly = 1) { dao.updateOrInsertPersoninfo(any(), any(), any(), any(), any(), any(), any()) }
    }
